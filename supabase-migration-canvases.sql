-- CollabCanvas Database Migration: Multi-Canvas Support
-- This migration adds support for multiple saved canvases
...
-- =======================
-- STEP 5: Create RLS policies for canvases
-- =======================
...
DECLARE
    ...
    canvas_uuid UUID;
BEGIN
    -- Loop through each unique room_id in shapes table
    FOR room_record IN 
    ...
    LOOP
        ...
        -- Update all shapes in this room to reference the new canvas
...
DROP FUNCTION create_default_canvases();

-- =======================
-- STEP 7: Update shapes RLS policies
-- =======================

-- Enable RLS on shapes if not already enabled
ALTER TABLE public.shapes ENABLE ROW LEVEL SECURITY;

-- Drop existing shape policies
...
-- This can be tightened later when user ownership is fully implemented
CREATE POLICY "Enable read access for all shapes" ON public.shapes
    FOR SELECT USING (true);

CREATE POLICY "Enable insert for all shapes" ON public.shapes
    FOR INSERT WITH CHECK (true);

CREATE POLICY "Enable update for all shapes" ON public.shapes
    FOR UPDATE USING (true);

CREATE POLICY "Enable delete for all shapes" ON public.shapes
...
-- Add foreign key constraint to link shapes to canvases
-- Using ON DELETE CASCADE so deleting a canvas removes its shapes
...
-- Create helper functions for canvas operations
