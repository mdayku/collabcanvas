// AWS Lambda Function for CollabCanvas AI
...
export const handler = async (event) => {
  ...
  try {
    const { message, canvasState } = JSON.parse(event.body);
    ...
    if (groqKey) {
      try {
        ...
        console.error('[Lambda] Groq failed:', error);
    ...
    if (openaiKey) {
      try {
        ...
        console.error('[Lambda] OpenAI failed:', error);
      }
    }
...
// [Include the same callGroqAPI, callOpenAIAPI, and SYSTEM_PROMPT functions from your api/ai.js]
